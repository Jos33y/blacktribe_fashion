/*
 * BLACKTRIBE FASHION — NIGERIAN STATES & LGAs
 * All 36 states + FCT with Local Government Areas.
 * Used in checkout address form for dependent dropdowns.
 */
...
export const nigerianStates = [
  ...
  },
  {
    ...
    lgas: ['Demsa','Fufore','Ganye','Gayuk','Girei','Gombi','Grie','Hong','Jada','Lamurde','Madagali','Maiha','Mayo Belwa','Michika','Mubi North','Mubi South','Numan','Shelleng','Song','Toungo','Yola North','Yola South'],
  },
  {
  ...
  },
  {
    ...
    lgas: ['Aninri','Awgu','Enugu East','Enugu North','Enugu South','Ezeagu','Igbo Etiti','Igbo Eze North','Igbo Eze South','Isi Uzo','Nkanu East','Nkanu West','Nsukka','Oji River','Udenu','Udi','Uzouwani'],
  },
  {
...
/**
 * Get LGAs for a given state value.
 * Returns array of { value, label } objects for the Select component.
 */
export function getLGAsForState(stateValue) {
  const state = nigerianStates.find((s) => s.value === stateValue);
  if (!state) return [];
  return state.lgas.map((lga) => ({ value: lga, label: lga }));
}
